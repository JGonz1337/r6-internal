...
namespace jm {
    ...
    namespace detail {
        ...
        XORSTR_FORCEINLINE constexpr std::uintptr_t key8()
        {
        ...
        XORSTR_FORCEINLINE constexpr std::uintptr_t
            load_xored_str8(std::uintptr_t key, std::size_t idx, const CharT* str) noexcept
        {
            using cast_type = typename std::make_unsigned<CharT>::type;
            constexpr auto value_size = sizeof(CharT);
            constexpr auto idx_offset = 8 / value_size;

            std::uintptr_t value = key;
            for (std::size_t i = 0; i < idx_offset && i + idx * idx_offset < N; ++i)
                value ^=
                (std::uintptr_t{ static_cast<cast_type>(str[i + idx * idx_offset]) }
        ...
        XORSTR_FORCEINLINE std::uintptr_t load_from_reg(std::uintptr_t value) noexcept
        {
#if defined(__clang__) || defined(__GNUC__)
            asm("" : "=r"(value) : "0"(value) : );
            return value;
#else
            volatile std::uintptr_t reg = value;
            return reg;
#endif
        }
    ...
    template<class CharT, std::size_t Size, std::uintptr_t... Keys, std::size_t... Indices>
    class xor_string<CharT, Size, std::integer_sequence<std::uintptr_t, Keys...>, std::index_sequence<Indices...>> {
#ifndef JM_XORSTR_DISABLE_AVX_INTRINSICS
        constexpr static inline std::uintptr_t alignment = ((Size > 16) ? 32 : 16);
#else
        constexpr static inline std::uintptr_t alignment = 16;
#endif

        alignas(alignment) std::uintptr_t _storage[sizeof...(Keys)];
    ...
    public:
        ...
        XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
            : _storage{ ::jm::detail::load_from_reg((std::integral_constant<std::uintptr_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
        {}
        ...
        XORSTR_FORCEINLINE void crypt() noexcept
        {
            ...
            alignas(alignment)
                std::uintptr_t arr[]{ ::jm::detail::load_from_reg(Keys)... };
            std::uintptr_t* keys =
                (std::uintptr_t*)::jm::detail::load_from_reg((std::uintptr_t)arr);
#else
            alignas(alignment) std::uintptr_t keys[]{ ::jm::detail::load_from_reg(Keys)... };
#endif

#if defined(_M_ARM64) || defined(__aarch64__) || defined(_M_ARM) || defined(__arm__)
#if defined(__clang__)
            ((Indices >= sizeof(_storage) / 16 ? static_cast<void>(0) : __builtin_neon_vst1q_v(
                reinterpret_cast<uintptr_t*>(_storage) + Indices * 2,
                veorq_u64(__builtin_neon_vld1q_v(reinterpret_cast<const uintptr_t*>(_storage) + Indices * 2, 51),
                    __builtin_neon_vld1q_v(reinterpret_cast<const uintptr_t*>(keys) + Indices * 2, 51)),
                51)), ...);
#else // GCC, MSVC
            ((Indices >= sizeof(_storage) / 16 ? static_cast<void>(0) : vst1q_u64(
                reinterpret_cast<uintptr_t*>(_storage) + Indices * 2,
                veorq_u64(vld1q_u64(reinterpret_cast<const uintptr_t*>(_storage) + Indices * 2),
                    vld1q_u64(reinterpret_cast<const uintptr_t*>(keys) + Indices * 2)))), ...);
#endif
    ...
    xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>)->xor_string<
        ...
        Size,
        std::integer_sequence<std::uintptr_t, detail::key8<Indices>()...>,
